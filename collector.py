...
# Улучшенный паттерн для захвата всех частей VLESS
...
def process():
    ...
    final_list = []
    # Сортировка по странам (RU в начале, XX в конце)
    sorted_items = sorted(
        unique_configs.values(),
        key=lambda x: (0 if get_country_code(x['host'], x['old_remark']) == "RU" else (2 if get_country_code(x['host'], x['old_remark']) == "XX" else 1))
    )

    for index, data in enumerate(sorted_items, 1):
        country = get_country_code(data['host'], data['old_remark'])
        new_name = f"{country} #{index:03d}"
        final_list.append(f"{data['body']}#{new_name}")
